/**
 * Orchestrated WhatsApp Flow setup for Kavitha PG:
 *   1) Generate / reuse RSA keypair, write to .env
 *   2) Upload public key to Meta
 *   3) Create + publish flow with endpoint = ${BACKEND_URL}/api/flow-endpoint
 *
 * Requires BACKEND_URL to be set already (HTTPS — e.g. ngrok).
 *
...
(async () => {
  ...
  let flowId;
  try {
    const r = await meta.createFlow('Kavitha PG Welcome', ['OTHER'], { endpointUri });
    flowId = r.id;
