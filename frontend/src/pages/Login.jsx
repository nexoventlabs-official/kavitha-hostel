...
export default function Login({ setAuth }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('kpg_token', data.token);
      setAuth(data.user);
      nav('/', { replace: true });
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-brand-50 via-white to-brand-100">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="w-16 h-16 rounded-full bg-brand-700 text-white flex items-center justify-center text-3xl mx-auto shadow-lg">
            🏠
          </div>
          <h1 className="mt-4 text-2xl font-bold text-brand-900">Kavitha PG</h1>
